...
| 10 | Unique 700+ word content per page | ✅ Done |
...
- Default `SITE_URL = https://sagecreeklending.com`.
...
- Side fixes during refactor:
  ...
  - Fixed wrong brand leak "California Capital" → "Sage Creek Lending" in industry pages.
...
- 3 pages (`contact`, `check-eligibility`, `industry`) gained a canonical they didn't have before.
...
- Centralized every phone number — `index.tsx`, `check-eligibility.tsx`, `industry.$slug.tsx`, `pillar.$slug.tsx`, `long-beach.$suburb.tsx`, `long-beach.$suburb.$pillar.tsx` all now read from `SITE_CONFIG.phone` / `SITE_CONFIG.phoneHref`. (Pages used to have 3 different inconsistent numbers and one mismatched display vs `tel:` link.)
- `contact.tsx`:
  ...
  - Fixed timezone bug: hours displayed "EST" but the business is Long Beach (Pacific). Now "PT".
...
**What was actually done:**
- Created `src/lib/pillars-content.ts` — 38 unique long-form bodies (one per pillar) with real Long Beach context (Port-tied logistics, neighborhood retail corridors, restaurant clusters, healthcare specialty desks, etc.).
- Created `src/lib/industries-content.ts` — 10 unique long-form bodies (one per industry) tying each industry to specific Long Beach commercial geography and lender appetite.
...
- **Verified word counts after the work:** 38/38 pillars ≥ 700 words, 10/10 industries ≥ 700, 10/10 suburbs ≥ 700, home and hub already comfortably above target.
- `/contact` and `/check-eligibility` are form pages (not content pages) — `check-eligibility` is `noindex` so word count is irrelevant. `contact` is left as a contact page with the NAP block + form, which is the correct shape for that page type.
